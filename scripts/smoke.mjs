 ...
 *
 * Тут же страница настроек: она рендерится React хоста (19, как во Freelens
 * 2.x), и её кнопки действительно ставят и убирают `lc`. Ванильный Freelens
 * 2.x React на глобал не кладёт — тогда страницы быть не должно, а бандл
 * обязан загрузиться всё равно.
 *
...
const render = () => renderToStaticMarkup(React.createElement(page.components.Input));

assert.match(render(), /Not installed/, "страница настроек не показала состояние");
renderToStaticMarkup(React.createElement(page.components.Hint));
...
// ванильный Freelens 2.x: React на глобале нет — бандл грузится, страницы нет.
// Свежая копия модуля через query: первая уже прочитала глобал при загрузке
delete globalThis.FreelensExtensionApi.React;

const { default: Vanilla } = await import("../dist/renderer.js?vanilla");

assert.deepStrictEqual(new Vanilla().appPreferences, [], "страница без React хоста");
