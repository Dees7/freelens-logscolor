...
Страница показывает, где лежит команда, а если поставить её нельзя, то почему.
...
- Скрипт запускается системным `node`, а если его в PATH нет — бинарником самого приложения с
  `ELECTRON_RUN_AS_NODE=1`, так что отдельно ставить Node не нужно.
...
**Странице нужен React от приложения.** Свой React в бандл не кладётся: второй экземпляр в
одном дереве ломает хуки, поэтому расширение берёт `React` с `globalThis.FreelensExtensionApi`.
Апстримный Freelens 2.x туда его пока не кладёт — там только `Common` и `Renderer`. На такой
сборке страницы просто нет, а раскраска логов работает как раньше: расширение по-прежнему
ставится на **ванильный** Freelens 2.x. Страница появляется на сборке, которая кладёт туда и
`React` (хостовая половина контракта, который уже предполагает `fixture-extension` в
репозитории Freelens). Без страницы `lc` можно поставить руками: `"cli": true` в
`~/.freelens/freelens-logscolor.json` и перезапуск приложения.
