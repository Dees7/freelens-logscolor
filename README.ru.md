...
Logs у пода. В приложении больше ничего не меняется: ни пунктов меню, ни колонок. По желанию
расширение ставит ещё команду `lc`, которая так же красит ваши логи в терминале.
...
снесёт установленную копию перед распаковкой новой. **После этого Freelens надо
...
terminal command*. На странице **Preferences → Extensions → freelens-logscolor** видно, где
лежит команда, а если поставить её нельзя, то почему.
...
Почему палитра, а не кнопки: ванильный Freelens 2.x React расширениям не отдаёт (на
`globalThis.FreelensExtensionApi` только `Common` и `Renderer`), а свой React в бандле был бы
вторым экземпляром в дереве приложения и ломал бы хуки. React-компонентов у расширения нет
вовсе — страница настроек отдаёт простой текст, — поэтому оно по-прежнему ставится на
**ванильный** Freelens 2.x, без патчей. В ветке `v1` хост React отдаёт, и там на странице есть
кнопки.
